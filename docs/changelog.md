...
- **`StrUtil`** +49 方法：
  - 比较与判断（10）：`equals_any`、`equals_any_ignore_case`、`equals_char_at`、`contains_only`、`has_letter`、`is_sub_equals`、`is_surround`、`is_wrap`、`is_lower_case`、`is_upper_case`、`is_all_char_match`
  - 公共前缀/后缀（5）：`common_prefix`、`common_suffix`、`compare`、`compare_ignore_case`、`concat`
  - 截取与格式化（8）：`brief`、`max_length`、`fix_length`、`hide`、`move`、`normalize`、`total_length`、`indexed_format`
  - 包裹与填充（7）：`wrap`、`wrap_if_missing`、`wrap_all`、`wrap_all_if_missing`、`pad_after`、`pad_pre`、`repeat_by_length`
  - 替换与移除（5）：`replace_ignore_case`、`replace_last`、`remove_all_prefix`、`remove_all_suffix`、`remove_suf_and_lower_first`
  - 分割与转换（5）：`split_trim`、`strip_all`、`swap_case`、`to_symbol_case`、`trim_to_null`
  - 空值处理与杂项（3）：`empty_if_null`、`desensitized`、`compare_version`
...
- **`DateUtil`** +33 方法：
  - 判断（5）：`is_last_day_of_month`、`is_expired`、`is_overlap`、`is_between`、`date_trunc`
  - 属性（7）：`day_of_year`、`length_of_month`、`length_of_year`、`millisecond`、`get_zodiac`、`get_chinese_zodiac`、`get_week`
  - 时间跨度（4）：`get_monthspan`、`get_weekspan`、`get_quarterspan`、`get_yearspan`
  - 操作与转换（7）：`compare`、`convert_timezone`、`format_chinese_date`、`month_add`、`convert_to_date`、`convert_to_datetime`、`age_by_birthday`
  - RFC 格式（4）：`rfc3339_date`、`rfc3339_date_parse`、`rfc2616_date`、`rfc2616_date_parse`
  - 实用（6）：`is_same_month`、`is_same_week`、`time_ago`、`iso_timestamp`、`format_between`、`between_day`
- **`BeanUtil`** +6 方法：
